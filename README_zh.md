...
X-Ray 默认采用轻量、适合低性能设备的流程：

1. **轻量 metadata 种子**：`获取 AI 数据` 首先只根据书名和作者生成紧凑 X-Ray 种子，避免打开书后扫描全文。
2. **当前位置上下文增强**：读到新人物或新情节后，使用 **当前位置上下文增强**，把当前阅读位置附近的短文本合并进缓存。
3. **AI 问答与角色合并**：从菜单或选中文本发起聚焦问题，再把有用的角色结果加入缓存。
4. **高级本地扫描**：本地候选和分块全文分析保留为手动 fallback，用于 metadata 和附近上下文不足的书。

打开书自动生成 X-Ray 种子也只走轻量 metadata 路径，不会在打开书时自动启动本地候选或分块全文扫描。

### PDF 行为

* 即使启用了 `auto_metadata_on_open`，PDF 打开时也会跳过自动种子任务。
* PDF 的手动功能仍然保留：轻量种子、AI 问答、当前位置上下文增强和高级扫描都可以从菜单手动启动。
* PDF 高级扫描带有页数和字符数上限，避免低性能阅读器在文本抽取时长时间卡住。
...
* 提供文本提取诊断
* 支持取消、重试、降级轻量种子和恢复可恢复的任务状态
* 可显示 HTTP 状态、错误细节、请求/响应大小和 OpenAI 兼容参数降级重试信息

#### 🔬 高级扫描

* 本地候选和分块全文分析是高级 fallback，不属于默认路径
* 文本抽取诊断只做轻量采样，不默认扫描整本书
* PDF 和 page-based 文档使用更严格的页数和字符数上限
* 大角色、地点、时间轴和历史人物列表采用短预览、分页和搜索优先展示
...
可选 v7 字段还包括 `seed_mode`、`enrichment_history`、`last_provider_id`、`last_model`、`source_stats` 和 `last_error`。旧缓存继续兼容。

### 后台任务状态

后台任务状态保存在书籍 sidecar 目录的 `xray_job_state.lua`。新状态可包含 `kind`、`retry_count`、`last_error_code`、`last_error_detail`、`request_size`、`response_size`、`status_code`、`provider_type` 和兼容参数降级重试信息。崩溃后残留的 `running` 状态会按可恢复任务处理，不会永久阻塞重试。

### 性能保护

* 打开书和构建菜单阶段避免重文本扫描。
* 文本抽取诊断只采样当前区域和少量页面。
* 当前位置上下文增强使用可配置的紧凑文本窗口。
* PDF 和 page-based 高级扫描有页数和字符数上限。
* 章节角色分析限制抽取文本长度和扫描角色数量。
* 大角色、地点、时间轴和历史人物列表先显示短预览，并优先提供分页/搜索。
...
**问：PDF 开启自动种子后为什么没有反应？**
答：这是预期行为。PDF 自动触发会被跳过，避免打开 PDF 时触发耗时文本抽取。需要时仍可从菜单手动运行轻量种子、AI 问答或高级扫描。

**问：后台任务失败怎么办？**
答：进入 **菜单 → X-Ray → 后台 AI 任务** 查看状态、Prompt 预览、HTTP/错误细节、兼容参数降级信息和文本提取诊断；必要时重试、恢复或降级为轻量种子。
